    ...
    public void onStateChanged(@NonNull LifecycleOwner source, @NonNull Lifecycle.Event event)
    {
        switch (event)
        {
            case ON_CREATE:
                break;
            case ON_START:
                Timber.tag("LIFE").d("DailyWallet into foreground");
                handler.postDelayed(() -> {
                    if (viewModel != null)
                    {
            ...
            case ON_PAUSE:
                break;
            case ON_STOP:
                Timber.tag("LIFE").d("DailyWallet into background");
    ...
    private void checkWarnings()
    {
        if (updatePrompt)
        {
            ...
            if (warns < 3)
            {
                ...
                cDialog.setTitle(R.string.dailywallet_update);
                cDialog.setCancelable(true);
                cDialog.setSmallText("Using an old version of DailyWallet. Please update from the Play Store or DailyWallet website.");
